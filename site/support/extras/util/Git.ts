...
export default class Git {
	...
	execSSHAgent(command: string, args: string[], options: any = {}): Promise<ChildProcess> {
		if (this.hasDeployCredentials()) {
			...
			let relativeDeployKey = options.cwd ? relative(options.cwd, deployKey) : deployKey;
			// 修复在 windows 上路径不兼容的问题
			relativeDeployKey = relativeDeployKey.replace('\\', '/');
			chmodSync(deployKey, '600');
			return promiseExec(`ssh-agent bash -c 'ssh-add ${relativeDeployKey };pwd;  ${ command } ${ args.join(' ') }'`, options);
		}
		else {
