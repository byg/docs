...
export = function (grunt: IGrunt) {
    ...
    const tasksExtrasDirectory = join(__dirname, '..', 'extras' ,'tasks');
    readdirSync(tasksExtrasDirectory).filter(function (path) {
        return extname(path) === '.ts';
    }).forEach(function (file) {
        const mid = join(tasksExtrasDirectory, basename(file, '.ts'));
        require(mid)(grunt);
    });
